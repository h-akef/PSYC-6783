...
* Decided against Allahu A'lam -- not enough occurrences to matter
* Completed code for converted all phrases in all files and also detecting the phrases after conversion for confirmation
* Updated analysis process:
data set: 19 interviews with questions removed
  + run R code to replace 4 common phrases with the dashed phrases: inshaaAllah, alhamdulillah, subhanAllah, mashaaAllah. Needed to rerun to include misspellings of mashaaAllah
    - after running code with flattened data, it turns out the new lines were not converted into spaces so words were mushed together, so ran again with additional str_flatten parameter collapse = " "
  + run detection
  + MEH:
    - from MEH frequency output of original files, conversion list was compiled based on different forms of selected key words
    - add limitation of potential words having different meaning in context (أصلي)
    - I opted not to add any more stop words to the list on MEH, so as no to accidentally exclude something
    - after a run, also added conversion words for وإن، فإن، فالحمد، و الحمد
    - there were still a couple versions of alhamdulillah that were mispelled for mashaaAllah and alhamdulilah, added code to convert
    - run MEH with edited transcript files
    - after identifying phrases/words of interest from initial output, MEH run again with only dictionary list of phrases/words of interest -- all content words with total freq>20
    - reviewed and labeled top frequent words as function/content up until 20 occurrences
  + create data frame with each row being an interview with the following columns: 
    - interview ID, parent gender, child gender(s) 
    - qualitative codes: religionXmorality separation, approaches to moral socialization, how religion is employed with moral socialization, religious socialization 
    - MEH word frequency for: words referring to religion, words referring to God, dashed religious phrases (as mentioned above)
  + plot qualitative codes (binary variables) with MEH word frequencies
  + potential statistical tests to run: t-test comparing mean word frequencies based on religionXmorality separation, regression with MEH word frequencies as outcome and religionXmorality separation and gender as predictors (all while acknowledging small sample size)
...
* Add phrase Allahu a'lam?
**_______________________________________________________**

# Date: Friday 4/22/22

* Wrote and tested code to convert different variations of inshaaAllah, then convert them into dashed phrase. Other phrases don't have common variations. Now adding all to loop.
* Final Analysis:
  + data set: 19 interviews with questions removed
  + run R code to replace 4 common phrases with the dashed phrases: inshaaAllah, alhamdulillah, subhanAllah, mashaaAllah.
  + run MEH with edited transcript files
  + create data frame with each row being an interview with the following columns: 
    - interview ID, parent gender, child gender(s) 
    - qualitative codes: religionXmorality separation, approaches to moral socialization, how religion is employed with moral socialization, religious socialization 
...
# Date Tuesday 4/19/22

* need to write code to make inshaaAllah is properly written, and the first two words aren't combined, also variations of the first word.
...
* I tested the phrase converter function to loop on a set of documents for a single phrase (inshaaAllah) and it works! The function also removes extra spaces (replaces double spaces with single spaces) See Codebook function "convert phrases in multiple files". This codebook file is also now available on the GitHub repo.
* Next I need to add additional phrases to convert in the same function
* Although I can write Arabic in RStudio, it's often wonky in relation to the cursor and where exactly it is. Also realized that writing Arabic text with quotation marks in a text file then copying it here doesn't identify the quotation marks as such for some reason. Arabic with quotations must be typed directly into RStudio. 
* After running the phrase converter function on a few transcript files, I also tested MEH again with the new files to make sure the dashed phrases are identified as a single word -- success! 
* Next, I need to check if a word is in the conversion list, and it's within the dashed phrase -- will that also be converted? or does it only convert exact matches of the word? I don't think it will make a difference in this particular instance, but I'm wondering. 

### GitHub: Creating repo from current project

* I updated the .gitignore file so that it ignores certain file extensions (.txt,.csv,.pdf...). This is mainly so that transcript files are not tracked for security reasons. I'm not sure what will happen when I create a repo on GitHub from this project. We'll see!
* Success! I ran use_github(), and the files with extensions in the .gitignore were not imported into the repo on GitHub. Now I don't need to worry about transcript files being publicly available online! (at least as far as I understand)
* The html file was still included though... I'm not sure why. 
* Here's a link to the repository: https://github.com/h-akef/PSYC-6783 
...
arabicTest <- read_lines("UB_RTest.txt")
arabicTest # aaaand this worked!! yippee!!
arabicTest2 <- read_lines("UB06.txt") #bigger text file include some English words within Arabic text
